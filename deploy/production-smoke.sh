...
#
# WHAT `validate_responses` COSTS HERE: NOTHING, AND IT IS MEASURED
# ----------------------------------------------------------------------------
...
#
# WHY THE ROSTER RULE ABOVE CARRIES A SECOND CLAUSE
# ----------------------------------------------------
...
#      inherited, so an ambient `KIOSK_<APP>_DB` cannot redirect it.
    ...
    if [ -e "$marker" ]; then
      {
        ...
        echo "!! There is no override for this check."
      } >&2
      exit 3
    fi
  done
  ...
  if [ -z "${CI:-}" ] && [ "${KIOSK_SMOKE_I_AM_DISPOSABLE:-}" != "1" ]; then
    {
      ...
      echo "!! CI sets CI=true and passes the same variable explicitly."
    } >&2
    exit 3
  fi
}
...
# DISABLE_DATABASE_ENVIRONMENT_CHECK, and disarming the platform guard is
# exactly what the three controls above exist to prevent.
drop_smoke_db() {
  local db="$1" user="$2" pw="$3"
  case "$db" in
    *_smoke) ;;
    *)
      echo "!! REFUSING: '${db}' is not a *_smoke database — this script only ever drops throwaway ones." >&2
      exit 3
      ;;
  esac
  if [ -n "$pw" ]; then
...
# ── THE MEASUREMENT THE POSTURE ABOVE RESTS ON, MADE ON EVERY RUN ───────────
#
...
smoke_stylish() {
  ...
  # the faithful proxy-CSRF reproduction: Rails already trusts X-Forwarded-Proto from a
  ...
  # an ambiguous forwarded scheme) is the exact condition the class needs,
  ...
  # The PoW HMAC key is mandatory outside development/test — the app
  ...
  # never follow an ambient KIOSK_STYLISH_DB and must never be the deploy
  # name `kiosk_stylish_production`. `_smoke` is a name no deployment
  ...
  # surface the log and fail immediately.
  ready=false
  ...
  echo
  echo "── Assertion 1: GET / → 200 (catches eager-load Zeitwerk crashes) ──"
  ...
  echo "── Assertion 3: GET /kiosk/auth/assistants unauthenticated → 302 to sign-in (missing-column class), NOT 500 ──"
  ...
  if [ -z "$token" ]; then
    ...
    if [ "$signin_code" = "302" ] || [ "$signin_code" = "303" ]; then
      ...
      fail "sign-in POST expected 302/303 redirect, got $signin_code (422 = CSRF-Origin rejection)"
    fi
  ...
  echo "── Assertion 5: forged cleartext identity bearer → 401 ──"
  # An `agent:u-…:a-…:r-…` bearer names a user, an agent and a ROLE, and NOTHING
  # parses it: assistants authenticate with the kiosk-pop JWT the engine minted,
  # verified by the engine's own DefaultAgentIdp, and no demo overrides
  ...
  if [ "$forged_code" = "401" ]; then
    ...
    fail "forged bearer expected 401, got $forged_code (something is parsing self-asserted bearers — cross-tenant auth bypass!)"
  fi

  echo "── Assertion 6: forged human X-Staff-Session → 401 in production ──"
  ...
  if [ "$staff_code" = "401" ]; then
    ...
    fail "forged X-Staff-Session expected 401, got $staff_code (a role-carrying human stand-in is reachable in production — staff-role self-grant!)"
  fi
  ...
  if [ "$signpost_code" = "422" ] && printf '%s' "$signpost" | grep -q "invalid_authenticity_token"; then
    ...
    fail "JSON sign-in POST expected 422 + invalid_authenticity_token body, got $signpost_code / '$(printf '%s' "$signpost" | head -c 120)' (the signpost regressed to a bodyless error)"
  fi
  ...
  if [ "$signout_code" = "401" ] && printf '%s' "$signout" | grep -q "not_signed_in"; then
    ...
    fail "JSON sign-out DELETE expected 401 + not_signed_in body, got $signout_code / '$(printf '%s' "$signout" | head -c 120)'"
  fi

  echo "── Assertion 9: unknown path → 404 with a BODY (public/404.html) ──"
  ...
  if [ "$notfound_code" = "404" ] && [ "$notfound_len" -gt 0 ]; then
    ...
    fail "unknown path expected 404 with a non-empty body, got $notfound_code / ${notfound_len} bytes (no public/404.html → bodyless error)"
  fi

  echo "── Assertion 10: GET /robots.txt and /favicon.ico → 200 statics ──"
  ...
  # production.rb leaves it on (it only sets headers). Without those two files
  # every crawler hit raises ActionController::RoutingError into the production
  # journal; this pins the 200s so that noise cannot arrive.
  ...
  if [ "$robots_code" = "200" ] && [ "$favicon_code" = "200" ]; then
    ...
    fail "statics expected 200/200, got robots.txt=$robots_code favicon.ico=$favicon_code (a missing public/ static puts RoutingError noise in the journal)"
  fi
}
...
# A surface the stylish smoke does not reach.
# ─────────────────────────────────────────────────────────────────────────────
smoke_prove() {
  PORT="${PORT:-4140}"
  ...
  # proxy-CSRF reproduction as stylish's sign-in, applied to this app's ONLY form
  ...
  export RAILS_ENV=production
  export SECRET_KEY_BASE="${SECRET_KEY_BASE:-$(ruby -e 'require "securerandom"; print SecureRandom.hex(64)')}"
  # The broker refuses to boot in production without PROVE_KEY_PEM, and
  ...
  # `kiosk_prove_production` — see the stylish smoke above.
  ...
  # of the broker's four lib/ modules), surface the log and fail.
  ready=false
  for _ in $(seq 1 60); do
    if ! kill -0 "$SERVER_PID" 2>/dev/null; then
      echo "!! Server process exited during boot (eager-load crash?). Log:"
  ...
  echo
  echo "── Assertion 1: GET / → 200 (eager-load smoke — all four lib/ modules load) ──"
  code="$(curl -s -o /dev/null -w '%{http_code}' "${PROXY_HEADERS[@]}" "${BASE}/")"
  [ "$code" = "200" ] && pass "GET / → 200" || fail "GET / expected 200, got $code"
  ...
  echo "── Assertion 5: POST /verify (decline) behind the proxy → 200, NOT 422 (assume_ssl/CSRF-Origin) ──"
  # Browser https Origin, assume_ssl-only (no X-Forwarded-Proto): the exact
  # proxy-CSRF condition. A regression of config.assume_ssl flips this to 422.
  ...
  if [ "$post_code" = "200" ]; then
    ...
    fail "POST /verify expected 200, got $post_code (422 = CSRF-Origin rejection)"
  fi
  ...
  echo "── Assertion 7: unknown path → 404 with a BODY (public/404.html) ──"
  ...
  echo "── Assertion 8: GET /robots.txt and /favicon.ico → 200 statics ──"
...
# ─────────────────────────────────────────────────────────────────────────────
# tudu: the housemate board + the fleet's only open sign-up. The third surface,
# and the reason is in the COVERAGE header: `/shared` is the only HTML page in
# the fleet rendered from a hand-written SQL projection, and `/users/sign_up` is
# the only open Devise registration.
# ─────────────────────────────────────────────────────────────────────────────
smoke_tudu() {
  PORT="${PORT:-4141}"
  ...
  ORIGIN="https://${HOST}"
  PROXY_HEADERS=(-H "X-Forwarded-Proto: https" -H "Host: ${HOST}")
  # Same proxy-CSRF reproduction as stylish's sign-in: browser https Origin, and
  ...
  # PoW secret and refuses to boot without them (same as stylish).
  export KIOSK_SIGNING_KEY_B64="${KIOSK_SIGNING_KEY_B64:-$(ruby -e 'require "openssl"; require "base64"; print Base64.strict_encode64(OpenSSL::PKey::RSA.new(2048).to_pem)')}"
  export KIOSK_ISSUER="${KIOSK_ISSUER:-$ORIGIN}"
  export KIOSK_POW_SECRET="${KIOSK_POW_SECRET:-$(ruby -e 'require "securerandom"; print SecureRandom.hex(32)')}"
  ...
  # PINNED, not inherited, and a throwaway `_smoke` name rather than the deploy
  # `kiosk_tudu_production` — see the stylish smoke above.
  ...
  echo "── Assertion 2: GET /shared → 200 AND the hand-written projection RESOLVED (missing-column class) ──"
  ...
  if [ "$shared_code" = "200" ] && printf '%s' "$shared_body" | grep -q "Flat 3B" \
    ...
    fail "GET /shared expected 200 with 'Flat 3B' + 'shared by Alice', got $shared_code (the hand-written housemate_board projection did not resolve)"
  fi
  ...
  echo "── Assertion 5: JSON POST at the human sign-in form → 422 + the wrong-door JSON body ──"
  # The wrong-door signpost this app returns to a JSON-dialing assistant. Only
  # production renders it (dev serves the debug page).
  signpost="$(curl -s -X POST "${PROXY_HEADERS[@]}" \
    -H "Content-Type: application/json" -H "Accept: application/json" \
    --data '{"user":{"email":"probe@example.com","password":"probe"}}' \
    "${BASE}/users/sign_in")"
  ...
  echo "── Assertion 6: unknown path → 404 with a BODY (public/404.html) ──"
  notfound_len="$(curl -s -o /dev/null -w '%{size_download}' "${PROXY_HEADERS[@]}" \
    -H "Accept: text/html" "${BASE}/this-path-does-not-exist")"
  notfound_code="$(curl -s -o /dev/null -w '%{http_code}' "${PROXY_HEADERS[@]}" \
    -H "Accept: text/html" "${BASE}/this-path-does-not-exist")"
  ...
  echo "── Assertion 7: GET /robots.txt and /favicon.ico → 200 statics ──"
  robots_code="$(curl -s -o /dev/null -w '%{http_code}' "${PROXY_HEADERS[@]}" "${BASE}/robots.txt")"
  favicon_code="$(curl -s -o /dev/null -w '%{http_code}' "${PROXY_HEADERS[@]}" "${BASE}/favicon.ico")"
  if [ "$robots_code" = "200" ] && [ "$favicon_code" = "200" ]; then
    pass "GET /robots.txt → 200, GET /favicon.ico → 200"
