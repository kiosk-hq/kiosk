...
-- Housekeeping (optional): the prune cron may DELETE rows older than N days to
-- reclaim disk — aggregates only look back 10 min / 1 h / all-time-registered,
-- so old rows past the registered-count horizon can be trimmed if desired.
