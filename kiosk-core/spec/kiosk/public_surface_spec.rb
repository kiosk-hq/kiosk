...
# Pins the removal of dead seams (ledger K-068, K-069): these constants
...
RSpec.describe "Kiosk public surface" do
  it "does not define the removed CredentialBrokers seam (K-068)" do
    expect(Kiosk.const_defined?(:CredentialBrokers)).to be(false)
  end

  it "does not define the removed NotificationAdapter seam (K-069)" do
    expect(Kiosk.const_defined?(:NotificationAdapter)).to be(false)
  end

  it "does not define the removed Event value type (K-069)" do
