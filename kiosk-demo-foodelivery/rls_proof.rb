...
#      SessionContext returns BOTH orders.  This is the pre-fix no-op: the
#      login role is a PostgreSQL superuser, which bypasses RLS even with FORCE.
#
#   2. Enforced session for A — SessionContext sets app.current_user_id = A,
#      drops role to kiosk_foodelivery_app (NOBYPASSRLS, non-owner).  Raw
#      unscoped SELECT * FROM orders returns ONLY A's row; B's is filtered by
#      the RLS policy (user_id = kiosk.current_user_id()).
#
#   3. Enforced session for B — same but scoped to B; returns only B's row.
#
# NOTE: FORCE is emitted in rls_overlay.rb for production-fidelity (extends
# the guarantee to a non-superuser table owner).  In this demo, FORCE does not
# restrict the superuser login role — the effective DB backstop here comes from
# ENABLE + role-drop to kiosk_foodelivery_app (a non-owner, NOSUPERUSER,
# NOBYPASSRLS role).
...
menu_row = conn.execute("SELECT id, restaurant_id FROM menu_items LIMIT 1").first
abort "No menu_items found — run db:seed first (demo:rls does this)." unless menu_row
...
[
  ...
  conn.execute(<<~SQL)
    INSERT INTO orders
      (id, user_id, restaurant_id, menu_item_id, quantity, total_cents,
       delivery_address, status, created_at, updated_at)
    VALUES
      ('#{oid}', '#{uid}', #{restaurant_id}, #{menu_item_id}, 1, 1599,
       '#{addr}', 'placed', NOW(), NOW())
    ON CONFLICT (id) DO NOTHING
  SQL
end
...
# The login role (pirj, superuser) bypasses RLS: even with FORCE ROW LEVEL
# SECURITY and two policies in place, a superuser sees every row.
# This is the "no-op" state that motivates the role-drop approach.
...
Kiosk::Server::SessionContext.open(connection: conn, identity: identity_a) do
  # Inside the transaction: role is kiosk_foodelivery_app, GUC app.current_user_id = A.
