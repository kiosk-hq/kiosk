...
class Agent < ApplicationRecord
  ...
  # one spelling and silently refuses the other, inside the gate that decides
  # whether alcohol is sold. Re-implementing `->>` in Ruby to fix that would be
  # hand-rolling a Postgres operator inside a KYC gate, which is the K-724
  # mistake with a new spelling. So the operator stays Postgres', expressed as
  # Arel (a quoted key, not an interpolated fragment) rather than as a SQL
  # string.
