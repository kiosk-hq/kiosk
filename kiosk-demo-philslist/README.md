...
`demo:register` pays an Equihash toll on the way in. Neither task here needs any
...
**`rake demo:clock_spec`** runs `spec/board_clock_spec.rb` **twice**, under
...
**`rake demo:access_spec`** runs `spec/listing_access_spec.rb` over the whole
...
| `spec/board_clock_spec.rb` | The DB-free proof of the reader-clock rendering, run under two `TZ` values by `rake demo:clock_spec` |
| `spec/listing_access_spec.rb` | The DB-free proof of the owner-scoped refusal surface — the `listing_id` shape guard and the one sentence a miss earns — run by `rake demo:access_spec` |
| `lib/tasks/demo.rake` | `rake demo:clock_spec`, `:access_spec`, `demo:setup`, `:walkthrough`, `demo`, `:isolation`, `:redteam`, `:schema`, `:binding`, `:register` |
