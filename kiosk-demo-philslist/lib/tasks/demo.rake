...
namespace :demo do
  ...
  desc <<~DESC
    ...
    Exits 0 when all are BLOCKED (0 BREACH); exits 1 on any BREACH and on a
    battery that proved nothing; exits 2 when a beat could not be exercised and
...
namespace :demo do
  ...
  task schema: :setup do
    ...
    failures.concat(
      descriptor_example_failures(queries: query_specs, actions: action_specs, minimum: 4),
    )
