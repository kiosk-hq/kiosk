...
#
# The key is configured per environment, read in config/environments/*.rb
# into Rails.configuration.x.prove.key_pem:
...
#
# BROKER-ONLY: a sibling demo's Rails has no `x.prove`, so this file
...
module ProveKey
  ...
  # No fallback key: an unconfigured process must fail, not mint.
  def config
    ...
    raise <<~MSG
      ...
      (Rails.configuration.x.prove) and therefore only works
      ...
      the same config/dev_prove_key.pem without the Rails dependency.
    MSG
  end
  ...
  # the deploy origin kyc.demo.kiosk.tech; configured per environment.
  def issuer
    config.issuer
  end

  # The signing keypair, from per-environment config. Production's env
