Rails.application.configure do
  ...
  # from its ENV (fail-closed), and development ships no default at all — the
  # two-server harnesses pin the secret on both sides.
  ...
  # Operator-binding `aud` defaults (the operator_id handle) and the claim
  # `iss` — the specs assert against these registration-derived values.
  ...
  # which is why production refuses to boot without an explicit PROVE_KEY_PEM.
