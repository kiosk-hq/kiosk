...
- Authenticated `/kiosk/exec` calls (sql + run verbs)
- RLS-enforced data isolation (two users, two views of the same table)
...
2. **Authenticated SQL** — `POST /kiosk/exec` with `command: sql`, returning rows filtered by RLS
3. **Run an Action** — `command: run` invoking `book_appointment` (the demo's lone registered Action)
4. **RLS isolation** — same `SELECT` run as Alice vs Bob; each sees only their own
