...
# POST /kyc/callback — the broker → operator leg. The broker POSTs the signed
# anonymized claim here once the human approves. skooti:
#
...
#   2. correlates it to a PENDING request skooti initiated (the request_id it
...
#      must be skooti (a claim minted for another operator is rejected HERE, at
#      the operator, keeping the engine attestation/wire UNCHANGED);
...
class KycCallbackController < ActionController::API
  def create
    ...
    # Only a request skooti actually initiated and that is still pending is
    ...
    # Nonce echo (anti stale/replay): the callback must carry the nonce skooti
    ...
    # be addressed to skooti. A claim minted for a different operator is rejected
    # here — the engine attestation/wire stays UNCHANGED.
    ...
    # Subject binding: the claim's sub must equal the user_id skooti bound this
    # request to at intake. (The agent's later /agents/kyc submit re-checks sub
    # against the AUTHENTICATED agent — the primary IssuedKycJwsTheft defense.)
