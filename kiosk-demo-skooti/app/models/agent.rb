...
# the attributes an attestation granted) and the demo only reads it. Promoting
# it into the engine is a public-API decision, not a handler conversion.
class Agent < ApplicationRecord
  ...
  #
  # The question is an EXISTENCE test, and that is what makes it safe. There is
  # no value to extract: the grant IS the row, so this counts the required names
  # that are present and compares the count. Nothing here can answer NULL, and
  # no spelling of `true` reaches this side at all — the engine judges that once,
  # on the write, in Postgres (`jsonb_each(...) WHERE value = 'true'::jsonb`).
  #
