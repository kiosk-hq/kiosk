...
**5. Ship the App Clip** (`appclip/`) — a passive NFC/QR tag or a pushed link launches it; it reads no AI-assistant state, only carries the short-lived rental token from the AI assistant to the lock over BLE. What ships here is iOS only: an Xcode project (`SkootiDemo.xcodeproj`, `SkootiClip/`, `project.yml`). The Android equivalent is an Instant App, and it is not written — nothing in this repo builds one.
