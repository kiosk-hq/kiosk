...
def skooti_run_flow(flow_rb, env_str = "", env: {}, runner: "ruby")
  require "open3"
  require "shellwords"
...
namespace :demo do
  ...
  task :rideflow do
    ...
    run_flow = lambda do |extra_env = {}|
      ...
      skooti_run_flow(flow_rb, env_str)
    end
...
namespace :demo do
  ...
  task isolation: :setup do
    ...
    result = skooti_run_flow(flow_rb, "SERVER_URL=#{server_url} KIOSK_ISSUER=#{kiosk_issuer}")
...
namespace :demo do
  ...
  task schema: :setup do
    ...
    failures.concat(
      descriptor_example_failures(queries: queries, actions: actions, minimum: 4),
    )
  ...
  task kyc: :setup do
    ...
    ProveBrokerBoot.with_broker(operator_host: host) do |broker|
      ...
      driver_env = "SERVER_URL=#{server_url} KIOSK_ISSUER=#{kiosk_issuer} " \
                   "KIOSK_PROVE_BROKER_URL=#{broker[:broker_url]} " \
                   "KIOSK_PROVE_ISSUER=#{broker[:wiring]["KIOSK_PROVE_ISSUER"]}"
      result = skooti_run_flow(flow_rb, driver_env)
    end
