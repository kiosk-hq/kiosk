...
#
# skooti's SQL builds `'<value>'::uuid` casts from agent-supplied ids — the
# `reservation_id` arg of start_rental / rent_motorcycle, and the
# `{"reservation_id":…}` entry inside a signed cart mandate's line_items that
# ValidatingRentalProvider prices at capture. Postgres rejects a malformed
...
# wire-supplied id (getgrocery, hoteling, atablefor, tudu, philslist). Each demo
