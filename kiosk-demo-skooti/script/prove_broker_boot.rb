...
#
# Every trust anchor is wired EXPLICITLY here, and skooti ships no pinned dev
# ProveKey: the broker's public key is fetched from the running broker's own
# GET /prove_key.pem and handed to skooti as
# KIOSK_PROVE_PUBLIC_KEY_PEM, alongside the URLs, the shared intake secret and
# the callback host.
#
# THE FILE IS SPLIT THE WAY IT IS SO A GATE CAN HOLD THE SHARED HALF.
# getgrocery ships the same helper, and {with_broker} is ~55 of its ~64
...
# the few lines that genuinely differ are all OPERATOR IDENTITY. Declared whole
# it would have to be `:per_demo`, which bin/check-demo-copies defines as
# compared to NOTHING, so an edit to the poll or the teardown reaching one copy
# would be invisible. The operator half lives in {broker_operator_env},
...
module ProveBrokerBoot
  ...
  # PER-OPERATOR: the PEM the booted broker signs with. skooti pins
  ...
  # PER-OPERATOR: the env skooti's server + the drivers must carry to
  ...
  #
  # SHARED WITH getgrocery AND HELD THERE: everything below is operator-neutral
  # and bin/check-demo-copies compares it against getgrocery's copy modulo
  # comments. Anything that has to name this operator belongs in one of the
  # three per-demo methods above, not here — the keyword is `operator_host:`
  # rather than `skooti_host:` for the same reason: a per-operator NAME in a
  # shared signature would force the whole method to be declared per-demo.
  #
