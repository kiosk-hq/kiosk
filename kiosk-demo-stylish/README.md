...
| `app/services/stub_user_idp.rb` | Role-carrying **user**-IdP (SSO/Okta stand-in): an `X-Staff-Session` header names a staff member; the session identity carries their `staff_role` |
| `app/services/composite_user_idp.rb` | Composite user-IdP: the role-carrying StubUserIdp first, then the real Devise session |
