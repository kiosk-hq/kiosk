...
It used to be the answer nobody chose. The parse was stdlib `Time.iso8601`, which binds a zoneless string to whatever zone the **server process** happens to run in: measured, the same `2026-09-14T14:00:00` is `+11:00` under `TZ=Etc/GMT-11` and `-02:00` under `TZ=Etc/GMT+2`, thirteen hours apart, from an environment variable nobody sets deliberately — while the comment beside it claimed the value was read in UTC. A salon in one zone deployed to a box in another booked the wrong hour and said nothing about it.
...
**The clock decides on the way OUT too, and the column carries a zone.** Every verb that publishes an appointment instant — the `book_appointment` confirmation, its two `slot` refusals, `my_appointments`, `salon_calendar` — renders it through `SalonClock.publish`, so one booking is one string wherever you read it. It was not: the confirmation answered `…Z` while the catalogue's own example, computed on the salon's clock for the stated reason that «the example and the response agree», answered `…+02:00`. Underneath, `appointments.slot` is `timestamp with time zone`, the same as the instant columns in atablefor and getgrocery; it used to be zone-less, which was correct only because `ActiveRecord.default_timezone` is `:utc` — an invariant about instants should be held by the schema, not by a framework default an operator may change in one line.
