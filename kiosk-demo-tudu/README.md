...
| Path | What's there |
|---|---|
| `db/migrate/` | The canonical kiosk migrations: the full `kiosk:install` set — every demo ships that output unmodified, so reservations/mandates/settlements are installed here and never written — plus the post-install kiosk migrations the whole fleet carries at identical timestamps (today `drop_kiosk_settlement_raw_jws`; a kiosk schema change after install arrives as a NEW file, never as an edit to a shipped one), plus `create_tudu_domain` (lists/memberships/todos/invites) |
