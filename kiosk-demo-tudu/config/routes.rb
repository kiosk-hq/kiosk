...
Rails.application.routes.draw do
  ...
  # POST /kiosk/<action-name>. tudu hand-draws its routes rather than mounting
  # the engine (that IS the escape hatch the engine documents), so the pair the
  # engine would have drawn is written out here — and, like the engine's, LAST,
  # so every reserved line above wins by first-match and no operator verb can
  # shadow `schema` or the auth plane.
