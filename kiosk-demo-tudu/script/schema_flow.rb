...
#
# Boots against a running tudu server, authenticates with a StubIdp token, calls:
#   GET /kiosk/schema
...
# `GET <mount>/schema` answers `{verbs, queries, actions}` DIRECTLY — the 0.3
# `{ok, kind, value}` envelope was retired at the cutover.
schema_value = body || {}
