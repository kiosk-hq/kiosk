...
- `Kiosk::Pow::Equihash.solve(challenge)`, in `kiosk/pow/equihash/solver` — the client half of `verify`: runs the packaged solver on one challenge the provider minted and returns the proof nonce, raising `SolverError` with the solver's own message. Callers stop repeating the `Open3` invocation and the two error branches around it, and nothing outside this gem has to know where `solve.py` lives. It is a SEPARATE require on purpose: a provider verifies a proof on an unauthenticated register, and what it loads there must not be able to spawn a process.
- `Kiosk::Pow::Equihash::Difficulty` — the operator's `KIOSK_POW_DIFFICULTY` knob: `low` (n=96, k=5, sub-second) or `high` (the gem's own `DEFAULT_N`/`DEFAULT_K`), with `params`, `level`, `high?` and a `pow_notice` for a discovery document's owner block. An unset or unrecognised value always lands on `low`, so a typo cannot price a verb out of reach. The heavy level is read off the shipped defaults rather than written out again.
