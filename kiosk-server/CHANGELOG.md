...
- **BREAKING (small): `DeviceVerification.approve` now REQUIRES `role:` (K-1127).** The module's own header invites a provider to drive a bespoke consent page from these helpers, and the keyword defaulted to `nil` — so a host that took up the invitation and simply did not know about `role:` produced role-less bindings silently, indistinguishably from an origin that deliberately reports no role. Naming it is mandatory now; passing `nil` is still legal and still means the second of those two, so `registration_role`/absent binding is unchanged (ADR-0011's no-regression clause). Nothing in this repo relied on the default: the engine's own `DeviceVerifyController` has always passed the approving identity's role, and the other role-less `approve` — the one `LinkCode.mint` calls, legitimately, because a link row is minted BY the human and already carries theirs — is `DeviceAuthorization#approve`, a different method that this module never reaches for a `:link` row at all.

- **BREAKING: the claim ceremony no longer accepts a client-chosen `role`/`scope`, and takes the role from the approving human instead (K-072 — this entry cited K-1109 when written, which is an unrelated row; corrected in place 2026-08-27, K-1130).** `POST <mount>/oauth/device_authorization` is unauthenticated; it read the role from that request, checked only that `config.roles` contained it, and wrote it into the row and then into the issued JWT — a privilege-escalation primitive on any origin declaring more than one role, reachable by a stranger and rubber-stamped by whichever account holder approved. The parameter is now `400 invalid_request` (refused, not ignored: a silent drop leaves the caller believing it got what it asked for), `DeviceCodeGrant.start` no longer takes `requested_role:` so a claim row cannot be born with one, and `DeviceVerification.approve` gained `role:` — the approving human's `Identity#role`, which `DeviceVerifyController` passes and the durable store now persists on UPDATE. The rebind branch is covered by the same source, so the escalation is not available one ceremony later. Side effect worth naming: the claim direction now INHERITS the human's role at all, which roles-from-IdP always specified and only the link direction implemented — an owner claiming used to bind at `registration_role`. Hosts calling `DeviceVerification.approve` from their own consent page should pass `role:`.

- **The engine's binding consent page names the access it is handing over (K-072 — this entry cited K-1109 when written, which is an unrelated row; corrected in place 2026-08-27, K-1130).** The verify page showed the key's RFC 7638 fingerprint and when it asked, and nothing about what the assistant would be able to do — so an approval was informed about identity and blind about privilege. It now states that the assistant will act as the approver, and names the approver's role where the provider defines one. Every host inherits it; a host shipping its own `app/views/kiosk/server/device_verify/show.html.erb` override should add the same line.

- **The wire-dispatch guard answers an RFC 9457 problem document, not the retired 0.3 envelope (K-1092).** `Kiosk::Handler`'s `kiosk_require_wire_dispatch!` 404s a route an operator drew straight at a handler controller — and it renders that 404 to a CLIENT, since the guard returns early under sub-dispatch and nothing re-wraps it. It was the last shipped body emitting `{ok:false, error:{…}}`, the shape deleted with the endpoints that served it; it now emits the flat document the wire emits, `application/problem+json` and all, built by `Errors::NotFound` so the two cannot drift. The nested shape inside `kiosk_rescue_to_wire` is untouched: that one is the internal handler↔`HandlerDispatch` protocol, which the Executor decodes.

- **`SchemaDefinitions` emits DDL that can replay onto a database that already has the kiosk schema (K-1083).** The 2026-08-20 rebuild renumbered every kiosk-owned migration, so on any pre-existing database all six re-emitted files read as pending and replayed onto tables that were already there — measured, `db:migrate` aborts one step in with `PG::DuplicateTable` and strands every later migration, including any corrective one. Every `CREATE` is now guarded, and migration 002 additionally brings a pre-fold `agents` table up to shape (`ADD COLUMN IF NOT EXISTS` for the three columns K-646 folded into the create) rather than stepping over it. A guarded create accepts a drifted table where an unguarded one shouted, so `bin/check-migration-replay` diffs the replayed catalogue against `db/structure.sql` on every push: the loud failure moved earlier, it did not go away.

- **The manage-assistants page says so when a spending cap binds nothing (K-1014).** The cap editor is rendered on every origin, but the number it saves is consulted only where an operator wires a `config.spending_cap` seam, so a human could set a policy nothing enforced and the page said nothing about it. It now states that where the control is rendered; the field still submits, because setting the policy on an origin that may not be the one that charges is deliberate.

- **BREAKING: `Kiosk::Query` and `Kiosk::Action` are replaced by one `Kiosk::Handler` mixin (K-921).** A handler controller had a kind, so queries and actions could never share a class. They can now: `include Kiosk::Handler` and mark each declaration `kind :query` or `kind :action` — the kind is a property of the declaration, and a declaration without one is refused at class-body load rather than defaulted onto an HTTP method. A name declared as both kinds on one class is refused there too, the same rule `HandlerRegistrations` already applied across classes. Migration is mechanical: swap the include, add one `kind` line per verb.

- **The PoW gates refuse to mint a challenge at a difficulty no proof could satisfy (K-843).** `Challenge.issue` is algorithm-agnostic and signs whatever params it is handed, so a degenerate `registration_pow_params` — or a policy computing one — produced challenges that every honest solve failed, answered with `403 invalid proof of work`. `PowGate.enforce` now asks the registered backend first (`Kiosk::Reputation::Backends.valid_params?`) and raises `Errors::ConfigurationError` naming the setting instead. A NEW RAISE PATH: a host whose PoW parameters were already unsatisfiable now fails loudly at the gate rather than quietly on the client. Backends that do not implement `.valid_params?` are unconstrained and unaffected.
...
- **The canonical migration set is rebuilt from scratch: six `create` migrations, renumbered 001-006 (K-646).** `kiosk:install` used to emit ten ordinals of which one was retired, one created a `device_authorizations` table the next-but-two DROPPED and rebuilt, and three added columns to a table created two files earlier — so a fresh adopter ran a migration HISTORY to arrive at a schema the templates can state outright. Every amendment is folded into the `create` it amended: `kyc_verified_at`, `spending_cap_cents` and `human_label` are columns of 002's `agents` table, and 004 creates `device_authorizations` in the account-binding shape directly. `SchemaDefinitions.kyc_verified_at_sql`, `.agent_governance_columns_sql` and `.rebuild_device_authorizations_sql` are GONE rather than deprecated — a shim would be a second way to build the same schema, which is the divergence this rebuild exists to end. Renumbering was measured before it was done: eight live references to an old ordinal existed outside the artifacts the rewrite touches anyway, two of them already stale, so they were swept. Phil: «Поменяй шаблон генерации миграций и сделай миграции начисто в демо. прод базы можно дропнуть.»
