...
Reads `current_user` from the incoming request and returns a `Kiosk::Identity` value object the rest of the Kiosk pipeline keys off (RLS GUCs, audit log, Action gating). See design spec §6.4.
...
Each Kiosk token carries exactly one active role (spec §6.2). For a Devise-authenticated human:
...
- [Design spec §6.4 — Pluggable IdP](https://github.com/kiosk-hq/kiosk-meta) (private during pre-launch)
